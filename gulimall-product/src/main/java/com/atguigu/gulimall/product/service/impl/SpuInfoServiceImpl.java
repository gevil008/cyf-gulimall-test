...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void saveSpuInfo(SpuSaveVo vo) {
        ...
        ResultBean resultBean = couponFeignService.saveSpuBounds(spuBoundTo);
        ...
        if (skus != null && skus.size() > 0) {
            skus.forEach(item -> {
                ...
                if (skuReductionTo.getFullCount() > 0 || skuReductionTo.getFullPrice().compareTo(new BigDecimal("0")) == 1) {
                    ResultBean resultBean1 = couponFeignService.saveSkuReduction(skuReductionTo);
                    if (!resultBean1.isSuccessful()) {
    ...
    public void upSpu(Long spuId) {
        ...
        try {
            ResultBean skuHasStock = wareFeignService.getSkuHasStock(skuIdList);
            if (skuHasStock.isSuccessful()) {
                Object resultValue = skuHasStock.getResultValue();
                if (null != resultValue){
                    List<SkuHasStockVo> lists = JSON.parseArray(JSONObject.toJSONString(resultValue), SkuHasStockVo.class);
                    stockMap = lists.stream()
        ...
        ResultBean r = searchFeignService.productStatusUp(collect);
