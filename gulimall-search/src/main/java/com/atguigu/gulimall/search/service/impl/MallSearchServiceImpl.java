    ...
    private SearchResult buildSearchResult(SearchResponse response, SearchParam param) {
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                ResultBean r = productFeignService.attrInfo(Long.parseLong(s[0]));
                if (r.isSuccessful()) {
                    Object resultValue = r.getResultValue();
                    AttrResponseVo data = JSON.parseObject(JSONObject.toJSONString(resultValue), AttrResponseVo.class);
